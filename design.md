...
2. Query RSSI (sync, instant)
3. Call `measureLatency()` (async, ~500ms)
4. Call `measureThroughput()` (async, ~2s)
5. If `extended=true`, call `measurePacketLoss()` (async, ~500ms)
6. Aggregate all data into a `NetworkMeasurement` object
7. Return to JavaScript
...
// Get signal strength (instant OS query)
...
| **All RSSI in dBm** | Industry standard; consistent across platforms |
...
| **Android has cellular RSSI; iOS doesn't** | Reflects platform limitations (Apple's choice) |
...
**Android:**
- Can measure everything (wifi RSSI, cellular RSSI, signal generation)
- Requires: `INTERNET`, `ACCESS_NETWORK_STATE`, `ACCESS_WIFI_STATE`, `READ_PHONE_STATE`

**iOS:**
- **Cannot** measure cellular RSSI (Apple doesn't expose it)
- Can measure Wi-Fi RSSI via `NEHotspotNetwork` (iOS 14.1+)
- More limited signal data → scoring relies more on latency + throughput
